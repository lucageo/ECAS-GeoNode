...
def _verify_cas2(ticket, service):
    ...
    url = (urljoin(settings.CAS_SERVER_URL, 'TicketValidationService') + '?' +
           urlencode(params))
    page = urlopen(url)
    try:
        response = page.read()
        ...
        if tree[0].tag.endswith('authenticationSuccess'):
            ...
            ###### Get namespace #####
            nm_user = str(ElementTree.QName('https://ecas.ec.europa.eu/cas/schemas',tag='user'))
            nm_email = str(ElementTree.QName('https://ecas.ec.europa.eu/cas/schemas',tag='email'))
            nm_firstname =  str(ElementTree.QName('https://ecas.ec.europa.eu/cas/schemas',tag='firstName'))
            nm_lastname = str(ElementTree.QName('https://ecas.ec.europa.eu/cas/schemas',tag='lastName'))

            ###########################

            username = tree[0].find(nm_user).text

            firstname = tree[0].find(nm_firstname).text
            lastname = tree[0].find(nm_lastname).text
            email = tree[0].find(nm_email).text
            #email=''
            #lastname = ''
            #firstname = ''
            return username, email, firstname, lastname,  None
        else:
...
class CASBackend(object):
    ...
    def authenticate(self, ticket, service, request):
        ...
        username, email, firstname, lastname, attributes = _verify(ticket, service)
        if attributes:
        ...
        try:
            ...
            user = None
            extra_f = {'first_name':firstname,'last_name':lastname}
            try:
                user = User.objects.create_user(username, email=email, password=None, **extra_f)
                user.save()
